...
class AggregatedCandfile(object):
    KEYS = ["pointing_id", "beam_id", "beam_name", "source_name",
            ...
            "pepoch", "maxdm_ymw16", "dist_ymw16", "pics_meerkat_l_sband_combined_best_recall",
            "pics_palfa_meerkat_l_sband_best_fscore", "png_path", "metafile_path", "filterbank_path",
    ...
    def add_candidates(self, cand_archive, pics_cut, dm_cut, sn_cut):
        ...
        try:
            subset = candidates[ (candidates["dm_new"] > dm_cut) &
                                 ((candidates["pics_MeerKAT_L_SBAND_COMBINED_Best_Recall.pkl"] > pics_cut) |
                                  (candidates["pics_PALFA_MeerKAT_L_SBAND_Best_Fscore.pkl"] > pics_cut)) &
                                  (candidates["S/N_new"] > sn_cut)]
        except Exception as error:
            print(str(error))
            raise
        else:
            print("Found {} matches".format(len(subset)))
            print("_"*50)
        if len(subset) == 0:
        ...
        for _, candidate in subset.iterrows():
            ...
            # -- Classifier scores --
            data["pics_meerkat_l_sband_combined_best_recall"] = candidate["pics_MeerKAT_L_SBAND_COMBINED_Best_Recall.pkl"]
            data["pics_palfa_meerkat_l_sband_best_fscore"] = candidate["pics_PALFA_MeerKAT_L_SBAND_Best_Fscore.pkl"]
            # -- Data products --
...
if __name__ == "__main__":
    ...
    if not tarballs:
        ...
        print("Selector returned {} tarballs".format(len(tarballs)))
    if opts.split:
        pointings = {}
        ...
        #print("Pointings: {}".format(pointings))

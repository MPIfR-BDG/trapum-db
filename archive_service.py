...
class CopyThread(Thread):
    ...
    def handle_messsage(self):
        log.debug("{}: Waiting on request".format(self.__repr__()))
        ...
        log.debug("{}: Source size: {}".format(self.__repr__(), size))
        if self._disk.has_space_for(size):
            log.debug("{}: Disk has space for copy".format(self.__repr__()))
            ...
            try:
                ...
                log.info("{}: Exepected copy time {} seconds".format(
                ...
                self._performance = size / (end - start)
                ...
                request.disk = self._disk
            log.debug("{}: Passing request into queue".format(self.__repr__()))
            self._output_queue.put(request)
        else:
            log.debug("{}: Source size exceeds remaining disk volume".format(self.__repr__()))
            log.debug("{}: Returning request to input queue".format(self.__repr__()))
...
class CopyManager(object):
    ...
    def start(self):
        ...
        for disk in self._disks:
            ...
            thread = CopyThread(self._queue, self._output_queue, disk, self._disk_sem)
